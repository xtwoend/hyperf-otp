...
use OTPHP\TOTP;
use Xtwoend\HyperfOtp\Constants\DBStates;
use Xtwoend\HyperfOtp\Services\Responder;
use Xtwoend\HyperfOtp\Services\OtpService;
use Xtwoend\HyperfOtp\Services\Transporter;
use Xtwoend\HyperfOtp\Constants\StatusCodes;
use Xtwoend\HyperfOtp\Object\OtpRequestObject;
use Xtwoend\HyperfOtp\Constants\StatusMessages;
...
class OtpValidator
{
    ...
    private static function getUuidId(OtpRequestObject $request): string
    {
        try {
            ...
            // TODO: support TOTP https://tools.ietf.org/html/rfc6238
            if(config('otp.use_totp')) {
                $totp = TOTP::create(
                    $request->secret,
                    config('otp.timeout'),
                    'sha1',
        ...
        if (!empty($getData)) {
            if ($getData->otp == $request->otp) {
                ...
                return Responder::formatter([
                    ...
                    'message' => StatusMessages::OTP_VERIFIED,
