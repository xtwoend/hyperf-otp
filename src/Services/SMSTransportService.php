...
use Xtwoend\HyperfOtp\Services\Template;
use GuzzleHttp\Exception\RequestException;
use Xtwoend\HyperfOtp\Exceptions\InvalidMethodException;
...
class SMSTransportService implements TransportServiceInterface
{
    ...
    {
        try {
            $template = new Template(BASE_PATH . '/template/sms.stub');
            $template->set('otp' , $top)
                ->set('company', $company)
                ->set('service', $service);
    ...
    public function sendMessage($to, $message, $extra_params = null, $extra_headers = [])
    {
        ...
        try {
            ...
            $this->logger->info("OTP Validator: Number: {$number} SMS Gateway Response Code: {$this->responseCode}");
            $this->logger->info("OTP Validator: Number: {$number} SMS Gateway Response Body: \n {$this->response}");
            ...
            if ($e->hasResponse()) {
                ...
                $this->logger->error("OTP Validator: Number:{$number} SMS Gateway Response Code: {$this->responseCode}");
                $this->logger->error("OTP Validator: Number:{$number} SMS Gateway Response Body: \n { $this->response}");
